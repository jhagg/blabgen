...
function save_visit( $visit_data ) {
	...
	$insert_data = array(
		...
		// use $_SERVER instead of gethostname() to support virtual hosts
	...
	$insert_data['enter_time'] = date( 'Y-m-d H:i:s', $visit_data['start_date'] );
	$insert_data['leave_time'] = date( 'Y-m-d H:i:s', $visit_data['end_date'] );

	log_msg( LOG_DEBUG, 'inserting visit data: ' . implode( '; ', $insert_data ) . ' ...' );
	...
	$stmt->bind_param( 'ssssss',
		$insert_data['name'], $insert_data['company'], $insert_data['enter_time'],
		$insert_data['leave_time'], $insert_data['webhost'], $insert_data['webkey'] );
...
function validate_visit_data( $visit_data ) {
	...
	// end_date: valid date. either integer timestamp (unix time in milliseconds)
	...
	if ( empty( $receivers ) ) {
		...
		foreach ( $rs as $r ) {
			...
			$sql = sprintf( "SELECT * FROM info WHERE uname='%s' LIMIT 1",
				$db->real_escape_string( $r ) );

			if ( $result = $db->query( $sql ) ) {
				$rec = $result->fetch_assoc();
				if ( !$rec ) {
					$err[] = "Receiver username not found: $r";
				} else {
					$receivers[] = $rec;
				}
				$result->free();
...
function save_picture( $tmp_picture_fname, $new_picture_id ) {
	...
	log_msg( LOG_DEBUG, sprintf( 'Attempting to move picture: %s -> %s', $tmp_picture_fname, $target_filename ) );

	if ( !is_dir( conf('picture_dir') ) || !is_readable( conf('picture_dir') ) ) {
		throw new Http_error( 500,
			sprintf( 'Directory is not readable: %s.', conf('picture_dir') ) );
	}
	if ( !is_readable( $temp_filename ) ) {
		throw new Http_error( 500,
			sprintf( 'Temp picture not readable: %s', $temp_filename ) );
	}
	if ( !is_readable( dirname( $target_filename ) ) ) {
		mkdir( dirname( $target_filename ) );
	}
	if ( !rename( $temp_filename, $target_filename ) ) {
		throw new Http_error( 500, sprintf( 'Could not move picture: %s -> %s',
...
function print_visitor_badge( $visit_data ) {
	...
	$cmd = sprintf( conf('print_badge_cmd'), $picture_filename, $name, $company,
		$nr, $date, conf('printer_name') );
	exec_cmd( $cmd );
	chdir( $d );
}
